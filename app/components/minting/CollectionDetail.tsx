...
function PreviewCheckoutModal({ collectionId, templateId, onClose }: { collectionId: string; templateId: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 rounded-lg p-5 w-full max-w-3xl">
          ...
          {wallet ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-gray-600 dark:text-gray-400">Loading wallet client...</p>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className={cardStyles.base}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <img
            ...
            alt="Collection"
            className="w-12 h-12 rounded object-cover bg-gray-100 dark:bg-gray-800 cursor-zoom-in"
          ...
          <div>
            ...
            {info?.metadata?.description && <p className="text-xs text-gray-600 dark:text-gray-400">{info.metadata.description}</p>}
          </div>
        </div>
      ...
      {info && (
        <div className="mb-6">
          <div className="mb-2">
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">Collection Summary</h3>
          </div>
          <div className="p-4 border border-gray-200 dark:border-gray-800 rounded bg-white dark:bg-gray-900">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <div className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-500">Collection ID</div>
                <div className="font-mono text-sm text-gray-900 dark:text-gray-100 break-all">{info.id}</div>
              </div>
              <div>
                <div className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-500">Contract ({info.onChain?.chain || DEFAULT_CHAIN})</div>
                <div className="font-mono text-sm text-gray-900 dark:text-gray-100 max-w-full overflow-hidden">
            ...
            {info.payments && (
              <div className="mt-4">
                <div className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-500">Sales</div>
                <div className="text-sm text-gray-900 dark:text-gray-100">{info.payments.price} {info.payments.currency} → <span className="font-mono break-all">{info.payments.recipientAddress}</span></div>
              </div>
            )}
            <div className="mt-4 grid md:grid-cols-2 gap-6">
              <div>
                <div className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-500">Standard</div>
                <div className="text-sm text-gray-900 dark:text-gray-100">{info.onChain?.type || 'n/a'}</div>
              </div>
              <div>
                <div className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-500">Transferable</div>
                <div className="text-sm text-gray-900 dark:text-gray-100">{String(info.transferable)}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Templates section */}
      <div className="mb-6 border border-gray-200 dark:border-gray-800 rounded bg-white dark:bg-gray-900 p-4">
        ...
        {isLoading ? (
          ...
          <div className="text-sm text-gray-600 dark:text-gray-400">No templates.</div>
        ) : (
          <div className="space-y-2">
            {templates.map((t) => (
              <div key={t.templateId} className="border border-gray-200 dark:border-gray-800 rounded p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <img
                      src={resolveImageUrl(t.metadata?.image || t.metadata?.imageUrl)}
                      alt="Template"
                      className="w-10 h-10 rounded object-cover bg-gray-100 dark:bg-gray-800 cursor-zoom-in"
                      onError={(e) => { (e.currentTarget as HTMLImageElement).src = fallbackImage; }}
                      onClick={() => {
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-600 dark:text-gray-400">{t.metadata?.description || 'No description'}</div>
                      {is1155 && <div className="text-xxs text-gray-500 dark:text-gray-500 mt-1">Token ID: {t.onChain?.tokenId ?? '-'}</div>}
                      <div className="text-xxs text-gray-500 dark:text-gray-500">Supply: {t.supply?.minted ?? 0}/{(() => { const lim:any = t.supply?.limit; const num = Number(lim); return (lim == null || !Number.isFinite(num) || num <= 0) ? 'unlimited' : lim; })()}</div>
                    </div>
                  </div>
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <span className="text-xs text-gray-600 dark:text-gray-400">Page {pageT}</span>
      ...
      {/* NFTs section */}
      <div className="mb-2 border border-gray-200 dark:border-gray-800 rounded bg-white dark:bg-gray-900 p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">{is1155 ? 'SFTs' : 'NFTs'}</h3>
        </div>
        {isLoading ? (
          <div className="text-sm text-gray-600 dark:text-gray-400">Loading NFTs…</div>
        ) : nfts.length === 0 ? (
          <div className="text-sm text-gray-600 dark:text-gray-400">No NFTs.</div>
        ) : (
          <div className="space-y-2">
            {nfts.map((n) => (
              <details key={n.id} className="border border-gray-200 dark:border-gray-800 rounded">
                <summary className="p-3 text-sm cursor-pointer flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <img
                      src={resolveImageUrl(n.metadata?.image)}
                      alt={is1155 ? 'SFT' : 'NFT'}
                      className="w-8 h-8 rounded object-cover bg-gray-100 dark:bg-gray-800 cursor-zoom-in"
                      onError={(e) => { (e.currentTarget as HTMLImageElement).src = fallbackImage; }}
                      onClick={() => {
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xxs text-gray-600 dark:text-gray-400">Owner: {n.onChain?.owner ?? '-'}</span>
                    </div>
                  </div>
                  <span className="text-xxs text-gray-600 dark:text-gray-400">Token #{n.onChain?.tokenId ?? '-'}</span>
                </summary>
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <span className="text-xs text-gray-600 dark:text-gray-400">Page {pageN}</span>
      ...
      {previewSrc && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center" onClick={() => setPreviewSrc(null)}>
          <div className="relative max-w-5xl w-[90vw] max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            <button
              type="button"
              className="absolute -top-3 -right-3 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-full w-8 h-8 flex items-center justify-center shadow cursor-pointer"
...
function AddTemplateModal({ collectionId, is1155, onClose }: { collectionId: string; is1155: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 rounded-lg p-5 w-full max-w-lg">
        <div className="flex items-center justify-between mb-3">
        ...
        <div className="space-y-4">
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <label className="text-sm text-gray-700 dark:text-gray-300 md:col-span-2">
              ...
              <textarea rows={4} className="border p-2 rounded w-full text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" value={description} onChange={(e) => setDescription(e.target.value)} />
            </label>
            <label className="text-sm text-gray-700 dark:text-gray-300 md:col-span-2">
              <span className="block mb-1">Image URL</span>
              <input className="border p-2 rounded w-full text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" value={useDefaultImage ? defaultImage : imageUrl} onChange={(e) => setImageUrl(e.target.value)} disabled={useDefaultImage} placeholder="https://... or ipfs://..." />
            ...
            <div className="text-sm text-gray-700 dark:text-gray-300">
              ...
              <input className="border p-2 rounded w-full text-sm disabled:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:disabled:bg-gray-800" value={supplyLimit} onChange={(e) => setSupplyLimit(e.target.value)} disabled={!enableSupplyLimit} placeholder="e.g. 1" />
            </div>
            {is1155 && (
              <div className="text-sm text-gray-700 dark:text-gray-300">
                <label className="flex items-center gap-2 mb-1">
                ...
                <input className="border p-2 rounded w-full text-sm disabled:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:disabled:bg-gray-800" value={tokenId} onChange={(e) => setTokenId(e.target.value)} disabled={!enableTokenId} placeholder="Custom tokenId (optional)" />
              </div>
            )}
            <div className="text-sm text-gray-700 dark:text-gray-300 md:col-span-2">
              ...
              <input className="border p-2 rounded w-full text-sm disabled:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:disabled:bg-gray-800" value={customTemplateId} onChange={(e) => setCustomTemplateId(e.target.value)} disabled={!useCustomTemplateId} placeholder="Provide templateId to PUT a specific ID" />
            </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 rounded-lg p-5 w-full max-w-lg">
        ...
        {error && <div className={cardStyles.error}><p className="text-red-700">{error}</p></div>}
        <div className="space-y-4">
          <div className="flex items-center gap-4 border-b border-gray-200 dark:border-gray-800" role="tablist" aria-label="Edit template tabs">
            <button
              type="button"
              role="tab"
              aria-selected={activeTab === 'metadata'}
              onClick={() => setActiveTab('metadata')}
              className={`px-3 py-2 -mb-[1px] border-b-2 text-sm transition-colors ${activeTab === 'metadata' ? 'border-green-600 text-green-700 dark:text-green-500 font-semibold' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}`}
            >
              Metadata
            </button>
            <button
              ...
              className={`px-3 py-2 -mb-[1px] border-b-2 text-sm transition-colors ${activeTab === 'supply' ? 'border-green-600 text-green-700 dark:text-green-500 font-semibold' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}`}
            >
              Supply
            </button>
            {is1155 && (
              <button
                ...
                className={`px-3 py-2 -mb-[1px] border-b-2 text-sm transition-colors ${activeTab === 'token' ? 'border-green-600 text-green-700 dark:text-green-500 font-semibold' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}`}
              >
                Token
              </button>
            )}
          ...
          {activeTab === 'metadata' && (
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <label className="text-sm text-gray-700 dark:text-gray-300 md:col-span-2">
                <span className="block mb-1">Image URL</span>
                <input className="border p-2 rounded w-full text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" value={image} onChange={(e) => setImage(e.target.value)} placeholder="https://... or ipfs://..." />
              </label>
            </div>
          )}
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg p-5 w-full max-w-lg">
        ...
        {isSaving || status === 'pending' ? (
          <div className="py-8 text-center">
            ...
            <p className="mt-3 text-sm text-gray-700 dark:text-gray-300">Minting…</p>
            <p className="mt-1 text-xxs text-gray-500 dark:text-gray-500">This can take a few moments. We'll close when done.</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div>
              <div className="text-sm text-gray-700 dark:text-gray-300 mb-2">Recipient Type</div>
            ...
            {recipientType === 'address' && (
              <label className="text-sm text-gray-700 dark:text-gray-300 block">
                ...
                <input className="border p-2 rounded w-full text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="0x..." />
              </label>
            )}
            {recipientType === 'email' && (
              <label className="text-sm text-gray-700 dark:text-gray-300 block">
                <span className="block mb-1">Email</span>
                <input className="border p-2 rounded w-full text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="me@example.com" />
              </label>
            )}
            {recipientType === 'userId' && (
              <label className="text-sm text-gray-700 dark:text-gray-300 block">
                <span className="block mb-1">User ID</span>
                <input className="border p-2 rounded w-full text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" value={uid} onChange={(e) => setUid(e.target.value)} placeholder="your-user-id" />
              </label>
            )}
            {recipientType === 'twitter' && (
              <label className="text-sm text-gray-700 dark:text-gray-300 block">
                <span className="block mb-1">Twitter handle</span>
                <input className="border p-2 rounded w-full text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" value={handle} onChange={(e) => setHandle(e.target.value)} placeholder="jack" />
              </label>
            )}
